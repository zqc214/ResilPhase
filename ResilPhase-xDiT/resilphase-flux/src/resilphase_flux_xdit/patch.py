...
from .flux_forward import resilphase_xfuser_flux_forward
...
def _is_flux_transformer_like(transformer) -> bool:
...
def apply_resilphase(pipe_or_transformer, config: ResilPhaseConfig | None = None, **kwargs):
    ...
    if not _is_flux_transformer_like(transformer):
        raise TypeError("apply_resilphase expects an xFuser/Diffusers Flux pipeline or Flux transformer.")
    ...
    transformer.forward = MethodType(resilphase_xfuser_flux_forward, transformer)
