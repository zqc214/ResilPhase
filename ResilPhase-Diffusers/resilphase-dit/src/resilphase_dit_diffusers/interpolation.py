...
def get_phase_mapping_from_cache(cache_dic: Dict, current_step: int) -> float:
    ...
    if mapping_method == "balanced":
        ...
        linear_mapping = (current_step - t_mean) / max_distance
...
def lagrange_cache_init(cache_dic: Dict) -> None:
    if "historical_cache" not in cache_dic:
        cache_dic["historical_cache"] = {"steps": [], "features": []}


def store_historical_cache(cache_dic: Dict, current: Dict, feature: torch.Tensor) -> None:
    ...
    if len(cache_dic["historical_cache"]["steps"]) > max_cache_size:
        cache_dic["historical_cache"]["steps"] = cache_dic["historical_cache"]["steps"][-max_cache_size:]
        cache_dic["historical_cache"]["features"] = cache_dic["historical_cache"]["features"][-max_cache_size:]


def get_historical_cache_for_prediction(cache_dic: Dict, current: Dict) -> tuple[list[int], list[torch.Tensor]]:
...
def barycentric_lagrange_prediction(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    if len(historical_steps) == 0:
        raise RuntimeError("ResilPhase cache has no historical DiT block features.")
...
def update_lagrange_system_cache(cache_dic: Dict, current: Dict, feature: torch.Tensor) -> None:
