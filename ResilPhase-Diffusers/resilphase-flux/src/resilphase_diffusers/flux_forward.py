...
def _checkpoint_or_call(self, block, *args):
    if torch.is_grad_enabled() and self.gradient_checkpointing:
        ...
        return torch.utils.checkpoint.checkpoint(block, *args, **ckpt_kwargs)

    return block(*args)


def resilphase_flux_forward(
    ...
    if current["type"] == "full":
        ...
        for block in self.transformer_blocks:
            encoder_hidden_states, hidden_states = _checkpoint_or_call(
                self,
                block,
                hidden_states,
