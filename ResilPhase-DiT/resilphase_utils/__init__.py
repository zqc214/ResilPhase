...
def get_mapping_method(cache_dic: Dict) -> str:
    return cache_dic.get("mapping_method", "chebyshev")
...
def get_phase_mapping_from_cache(cache_dic: Dict, current_step: int) -> float:
    ...
    left_step = max(s for s in recent_steps_sorted if s <= current_step)
    right_step = min(s for s in recent_steps_sorted if s >= current_step)
...
def compute_barycentric_weights(phase_nodes: list) -> list:
    """
    计算重心权（barycentric weights）
    根据相位节点计算对应的重心权重
    
    公式: w_j = ∏_{k=0,k≠j} 1/(s_j - s_k)
    
    :param phase_nodes: 相位节点列表，例如 [1, 0, -1]
    :return: 对应的重心权重列表，例如 [0.5, -1, 0.5]
    """
    n = len(phase_nodes)
    if n == 0:
        return []
    ...
    for j in range(n):
        ...
        for k in range(n):
            if k != j:
                denominator = phase_nodes[j] - phase_nodes[k]
                if abs(denominator) < 1e-12:  # 防止除零错误
                    # 如果两个节点重合，设置权重为0
                    product = 0.0
                    break
...
def update_barycentric_weights_cache(cache_dic: Dict):
    """
    更新重心权缓存
    在相位轴缓存更新后，同时计算并缓存重心权
...
def get_barycentric_weights_from_cache(cache_dic: Dict) -> list:
    """
    从缓存中获取重心权
    
    :param cache_dic: Cache dictionary containing barycentric weights
    :return: 重心权列表
    """
    if 'phase_axis' not in cache_dic or 'barycentric_weights' not in cache_dic['phase_axis']:
        return []
    
    return cache_dic['phase_axis']['barycentric_weights']
...
def store_historical_cache(cache_dic: Dict, current: Dict, feature: torch.Tensor):
    ...
    :param feature: Current step's feature tensor (F_j) - 总变化量
    """
...
def barycentric_lagrange_prediction(cache_dic: Dict, current: Dict) -> torch.Tensor:
    """
    使用重心形式的拉格朗日插值进行预测 - 切比雪夫节点映射版本
    基于历史完整计算的缓存进行预测
    
    公式: F_predicted = P(s_target) = (∑_{j=0}^n w_j/(s_target - s_j) * F_j) / (∑_{j=0}^n w_j/(s_target - s_j))
    ...
    # 2. 获取当前时间步的相位轴映射 s_target（使用切比雪夫节点映射）
    ...
    for j in range(len(historical_features)):
        ...
        if abs(denominator_j) < 1e-12:
            ...
            print(f"Target phase {s_target} matches node {j} at phase {s_j}, returning exact value")
            return F_j
...
def update_lagrange_system_cache(cache_dic: Dict, current: Dict, feature: torch.Tensor):
    """
    更新拉格朗日插值系统的完整缓存 - 切比雪夫节点映射版本
    在完整计算时间步调用，同时更新相位轴、重心权和历史特征缓存
    
    :param cache_dic: Cache dictionary
    :param current: Current step information  
    :param feature: Current step's computed feature - 总变化量
    """
    # 1. 存储历史缓存
    store_historical_cache(cache_dic, current, feature)
    
    # 2. 更新相位轴和重心权缓存（使用切比雪夫节点映射）
    update_phase_axis_cache(cache_dic, current)
    
    # print(f"Updated Lagrange system cache (Chebyshev mapping) at step {current['step']}")
    
    # 获取历史缓存大小
    historical_cache_size = len(cache_dic.get('historical_cache', {}).get('steps', []))
    # print(f"Historical cache size: {historical_cache_size}")
    
    # 打印相位节点和重心权
    phase_nodes = cache_dic.get('phase_axis', {}).get('phase_nodes', [])
    barycentric_weights = cache_dic.get('phase_axis', {}).get('barycentric_weights', [])
    # print(f"Phase nodes (Chebyshev): {[round(x, 4) for x in phase_nodes]}")
    # print(f"Barycentric weights: {[round(x, 4) for x in barycentric_weights]}") 
