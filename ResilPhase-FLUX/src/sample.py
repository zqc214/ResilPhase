...
def main(opts: SamplingOptions):
    ...
    # Initialize list to store diffusion times for average calculation
    diffusion_times = []
    ...
    for batch_idx in range(num_prompt_batches):
        ...
        for image_idx in range(opts.num_images_per_prompt):
            ...
            # seed = base_seed
            print(f"Generating image {idx} with seed: {seed}")
            ...
            diffusion_time = diffusion_end_time - diffusion_start_time
            
            # Store diffusion time for average calculation
            diffusion_times.append(diffusion_time)
            
            print(f"Diffusion sampling time: {diffusion_time:.3f}s")
    ...
    # Calculate and print average diffusion sampling time
    if diffusion_times:
        avg_diffusion_time = sum(diffusion_times) / len(diffusion_times)
        print(f"\n=== Generation Summary ===")
        print(f"Total images generated: {len(diffusion_times)}")
        print(f"Average Diffusion sampling time: {avg_diffusion_time:.3f}s")
        print(f"Total Diffusion sampling time: {sum(diffusion_times):.3f}s")
