...
def save_image(
    ...
    # Skip NSFW check if classifier is not available
    if nsfw_classifier is not None:
        nsfw_score = [x["score"] for x in nsfw_classifier(img) if x["label"] == "nsfw"][0]
        nsfw_safe = nsfw_score < nsfw_threshold
    else:
        nsfw_safe = True  # Skip NSFW filtering when classifier is unavailable
...
def load_flow_model(
    ...
    if ckpt_path is not None:
        ...
        # # Move model from meta device to target device after loading weights
        # # Use to_empty() for meta tensors, then move to target device
        # model = model.to_empty(device=device).to(device)
...
def load_t5(device: str | torch.device = "cuda", max_length: int = 512) -> HFEmbedder:
    ...
    return HFEmbedder("/mnt/public/zqc/t5-v1_1-xxl", max_length=max_length, torch_dtype=torch.bfloat16).to(device)
...
def load_clip(device: str | torch.device = "cuda") -> HFEmbedder:
    ...
    return HFEmbedder("/mnt/public/zqc/clip-vit-large-patch14", max_length=77, torch_dtype=torch.bfloat16).to(device)
