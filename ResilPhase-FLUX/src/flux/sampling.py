...
def denoise_test_FLOPs(
    ...
    for t_curr, t_prev in zip(timesteps[:-1], timesteps[1:]):
        t_vec = torch.full((img.shape[0],), t_curr, dtype=img.dtype, device=img.device)
        current['t'] = t_curr
        # 为 FLOPs 计算准备输入参数（只包含tensor类型的参数）
        flops_inputs = dict(
            img=img,
            img_ids=img_ids,
            txt=txt,
        ...
        # 创建一个包装器模型用于 FLOPs 计算，自动提供 cache_dic 和 current 参数
        class FluxFLOPsWrapper(torch.nn.Module):
            def __init__(self, original_model, cache_dic, current):
                super().__init__()
        ...
        # 处理 meta tensor 问题：如果模型有 meta tensor，跳过 FLOPs 计算
        device = img.device  # 获取输入张量的设备
        
        # 检查模型是否有 meta tensor
        has_meta_tensor = any(param.device.type == 'meta' for param in model.parameters())
        if has_meta_tensor:
            # 如果有 meta tensor，跳过 FLOPs 计算，避免内存和兼容性问题
            print(f"Warning: Model contains meta tensors, skipping FLOPs calculation for step {current['step']}")
            step_flops = 0  # 直接设置为数值，不需要转换
        else:
            # 创建包装器模型
            wrapped_model = FluxFLOPsWrapper(model, cache_dic, current)
            
            flops, macs, params = calculate_flops(model=wrapped_model,
                                          kwargs = flops_inputs,
                                          print_results=False)
            # calflops 返回的是字符串格式，需要转换为数值
